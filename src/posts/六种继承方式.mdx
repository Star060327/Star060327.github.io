...
原理：直接利用原型链特征实现的继承，让构造函数的prototype（它本质也是一个对象，也是有**proto**属性的）指向另一个构造函数的实例。仅通过修改构造函数的prototype
...
1. 创建实例时不能传参数
2. 如果原型的属性是引用数据类型，所有的实例都会共享这个属性（引用数据类型在堆里），某个实例的修改可能影响其他实例。\
...
1. YellowRace构造函数、xiaoming和laowang实例都没有和Person的原型对象产生联系，因此子类不能访问父类原型上定义的方法，所有的父类都必须采用构造函数模式。
2. 由于父类必须在构造函数中定义方法，通过盗用构造函数继承的方法本质上都变成了实例自己的方法，不是公共的方法，因此失去了复用性。
...
原理：如果你有一个已知的对象，想在它的基础上再创建一个新对象，那么你只需要把已知对象传给object函数即可。
创建完成后，新对象的**proto**指向所传参数。
...
继承的本质就是子类通过原型链继承父亲的方法，通过构造函数调用继承父类的属性
