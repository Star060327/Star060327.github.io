...
const ReactPlayground:React.FC<PlaygroundProps>= ({maxHeight="800px",defaultShowConsole=false,initialFiles=DEFAULT_FILES}) => {
  ...
  <div className={styles['edit-content']} ref={editContainerRef}>
        ...
        <ReactEdit activeFile={activeFile} onChange={(content:string)=>{
  ...
  return (
    <ReactPlaygroundLayout
      ...
      previewComponent={
        <ReactPreview files={files} logs={logs} addLog={addLog} clearLog={clearLog} showLog={showLog}></ReactPreview>
      }
    />
  )
}
