...
export function handleVueEditorWillMount(monaco: Monaco) {
  ...
  monaco.languages.setMonarchTokensProvider('vue', {
    ...
    tokenizer: {
      root: [
        // 1. Script 标签 (JS/TS)
        [/<script\s*lang="ts"\s*>/, { token: 'tag', next: '@scriptEmbedded.ts' }],
        [/<script(\s+(setup|lang="ts"))*>/, { token: 'tag', next: '@scriptEmbedded.js' }],
      ...
      // --- Vue 插值 {{ ... }} ---
      interpolation: [[/}}/, { token: 'delimiter', next: '@pop' }], { include: '@jsContent' }],
