...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
      selections.forEach((selection: any) => {
        ...
        for (let i = startLine; i <= endLine; i++) {
          ...
          // 检查是否已注释
          const isCommented = context === "template" || context === "style"
            ? trimmedContent.startsWith(commentStart.trim()) && trimmedContent.endsWith(commentEnd.trim())
          ...
          if (isCommented) {
            ...
            if (context === "script") {
               ...
               const inner = trimmedContent
                 .substring(commentStart.trim().length, trimmedContent.length - commentEnd.trim().length)
                 .trim();
