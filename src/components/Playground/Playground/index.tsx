...
import {usePlayground,type File,DEFAULT_FILES} from '../hooks/usePlayground'
...
const Playground:React.FC<PlaygroundProps>= ({maxHeight="800px",defaultShowConsole=false,initialFiles=DEFAULT_FILES}) => {
  ...
  <div className={styles['edit-content']} ref={editContainerRef}>
        ...
        <Edit activeFile={activeFile} onChange={(content:string)=>{
  ...
  return (
    <PlaygroundLayout
      ...
      previewComponent={
        <Preview files={files} logs={logs} addLog={addLog} clearLog={clearLog} showLog={showLog}></Preview>
      }
    />
  )
}
